...
app_ui = ui.page_fluid(
    ui.head_content(
        ...
        ui.tags.script("""
            ...
            let activeFeatureId = null;  // Track the currently active feature
            ...
            function generateLayers(data, colors) {
                ...
                if (data.sidewalks && colors.length > 0) {
                    layers.push(
                        new deck.GeoJsonLayer({
                            ...
                            loadOptions: {
                                fetch: {
                                    maxRequests: 6,  // Reduce concurrent requests
                                    throttleRequests: true
                                }
                            },
                            transitions: {
                            ...
                            refinementStrategy: 'never',  // Changed from no-overlap
                            tileSize: 512,  // Increased from 256
                            maxZoom: 20,
                            minZoom: 10,
                            extent: [-74.2, 40.4, -73.7, 40.9],
                            getTileData: ({id, data}) => data,
                            _subLayerProps: {
                                'geojson-layer': {
                                    culling: true,
                        ...
                        new deck.GeoJsonLayer({
                            ...
                            onClick: (info) => {
                                if (info.object) {
                                    ...
                                    // If clicking the same feature that's already active
                                    if (activeFeatureId === featureId) {
                                        // Close the popup and reset the active feature
                                        if (activePopup) {
                                    ...
                                    // Trigger a redraw of the deployment layer to update colors
                                    deckgl.setProps({
                ...
                function initDeck(data, colors) {
                    // In your initDeck function, modify the mapgl initialization:
                    if (!mapgl) {
                        ...
                        mapgl.on('load', () => {
                            ...
                            // Add navigation controls with rotation and pitch
                            mapgl.addControl(
                                new maplibregl.NavigationControl({
                                    visualizePitch: true,  // Shows the pitch control
                                    showZoom: true,        // Shows the zoom controls
                                    showCompass: true      // Shows the compass/rotation control
                                }),
                                'top-right'
                            );
